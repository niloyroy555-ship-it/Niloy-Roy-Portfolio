...
export default function Timeline() {
  ...
  return (
    <section id="experience" className="relative mx-auto max-w-5xl scroll-mt-24 px-5 py-24 md:px-8 lg:py-32">
      <div className="mb-16">
        <Reveal>
      ...
      <div ref={ref} className="relative pl-8 lg:pl-0">
        {/* center line */}
        <div className="absolute left-[7px] top-0 h-full w-px bg-fg/10 lg:left-1/2 lg:-translate-x-1/2" />
        <motion.div
          style={{ scaleY, transformOrigin: 'top' }}
          className="absolute left-[7px] top-0 h-full w-px bg-gradient-to-b from-brand via-violet2 to-transparent lg:left-1/2 lg:-translate-x-1/2"
        />
        ...
        <div className="space-y-10 lg:space-y-2">
          {experience.map((job, i) => {
            const right = i % 2 === 1
            return (
              <div key={i} className="relative lg:grid lg:grid-cols-2 lg:gap-10">
                {/* node */}
                <span className="absolute -left-[26px] top-1.5 z-10 grid h-4 w-4 place-items-center rounded-full bg-base lg:left-1/2 lg:-translate-x-1/2">
                  <span className="h-2 w-2 rounded-full bg-brand shadow-[0_0_14px_rgba(109,141,255,0.9)]" />
                </span>

                {coarse ? (
                  <div
                    className={`rounded-[1.75rem] glass-card p-6 lg:my-4 ${right ? 'lg:col-start-2' : 'lg:col-start-1 lg:text-right'}`}
                  >
                    ...
                    <div className={`mt-4 flex flex-wrap gap-2 ${right ? '' : 'lg:justify-end'}`}>
                      {job.tags.map((t) => (
